...
public class CreationFichierAudio {
	/**
	 * @param track
	 *            Un tableau d'octet décrivant le son
	 * @throws IOException
	 *             Si erreur d'écrture dans le fichier
	 */
	/*
	 * SOURCE:
	 * https://forum.processing.org/two/discussion/4339/how-to-save-a-wav-file-
	 * using-audiosystem-and-audioinputstream-of-javasound
	 */
	public static void saveWAV(Playable track) throws IOException {
		...
		AudioInputStream ais = new AudioInputStream(new ByteArrayInputStream(tabSon), frmt, tabSon.length);

		// Récupération de la date
		String strDate = new SimpleDateFormat("dd/MM/yyyy-HH:mm:ss", Locale.FRANCE).format(new Date());

		// Ecriture dans le fichier
		AudioSystem.write(ais, AudioFileFormat.Type.WAVE, new File(strDate + ".wav"));
	}
